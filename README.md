...
# JavaScript编码规范
## 注意，此文档是参考百度前端进行编写，参考文档 [ecomfe/spec](https://github.com/ecomfe/spec)
___
...
```javascript
function string_format(source) 
{
    
}
```

##### [强制] 函数的 `参数` 使用 `小写下划线命名法`。

示例：

```javascript
function hear(the_params)
{
...
##### [强制] `全局变量` 使用 `__`作为前缀，对象的是私有方法，使用 `_`作为前缀
...
demo_obj.prototype._init = function()
{
    alert('yes');
}
...
##### [强制] `枚举变量` 使用 `驼峰命名法`，`枚举的属性` 使用 `全部字母大写，单词间下划线分隔` 的命名方式。
...
##### [强制] `类名` 使用 `名词`+加上`Class`。

示例：

```javascript
function engineClass(options) 
{
